...
2. **API 계약 문서 작성 ↔ 디자인 시안 (병렬)** — 두 트랙은 서로 막지 않는다.
   ...
   - `design/`: 화면 시안. **이 항목(프론트엔드 UI/UX 시안 작업)을 시작하기 전에
     ...
     **reference-main.html은 메인/홈 화면만 다룬다** — 목록·상세류 화면을 그릴 때는 대신
     `reference/project-management-bundle.html`을 구현 예시로 참고한다(2026-09-03 추가, 근거:
     `ux-philosophy/` 기존 구현 예시가 메인 화면 하나뿐이라 목록·상세 화면을 그릴 때 참고할 게
     없었다). 이 파일은 리포 루트 `reference/`에 있는 **고정 스냅샷**이다 — 원본은
     `features/project-management/design/reference-proposal/`에서 담당자(유동우)가 계속
     갱신하지만, 여러 기능이 동시에 구현하는 스프린트 기간 동안 참고 기준이 흔들리지 않도록
     2026-09-03 시점 버전으로 얼려 두었다(`reference/README.md` 참고). 화면 10장 중 확정
     참고 대상은 7장뿐이다 — `experts.html`·`expert.html`·`guide.html` 3장은 ERD·PRD 근거가
     없어 향후 검토 대상으로 남겨두었으니 구현 근거로 쓰지 않는다(`reference/README.md`의 범위
     표 참고). 구조·원칙 참고용으로 쓰고 그대로 베끼지 않는다. 자세한 역할 구분은
     `ux-philosophy/AGENTS.md` 참고.
