...
6. **`feedback_loop/` — 내 기능 이름의 파일을 날짜 무관하게 전부 찾는다.** 통합 과정에서 내
   `spec.md`/`prototype/`에 없던 공백을 팀장(+AI)이 채웠거나, 다른 기능과 충돌해 한쪽으로
   맞춘 내용이 여기 남는다. 전체 규칙은 `feedback_loop/README.md`.

   ```bash
   grep -rn "상태: 미확인\|상태: 검토중\|상태: 재이슈" feedback_loop/*/ | grep {내 기능 이름}
   ```

   `상태: 반영완료`인 항목은 건너뛴다. **"가장 최근 날짜 폴더만" 보지 않는다** — 하루를
   건너뛰어도 놓치지 않으려면 미해결 상태를 기준으로 찾아야 한다.

   **찾았다면 원문을 그대로 사용자에게 보여주고 끝내지 않는다.** 담당자의 AI는 순서대로:
   ① 내용을 먼저 검토한다(무엇이 왜 그렇게 채워졌는지 이해한다) → ② 사용자에게 짧게 요약해
   전달한다(원문 전체를 붙여넣지 않는다) → ③ 어떻게 반영할지 방향을 근거와 함께 제안한다
   ("이대로 spec.md 규칙으로 편입하는 게 맞다" / "원래 의도와 달라 재이슈가 낫다") →
   ④ 사용자가 정하면 `상태:` 값을 바꾸고 작업한다 — 시작하면 `검토중`, 끝내면 `반영완료`,
   동의하지 않으면 `재이슈`. `재이슈`는 연관 담당자 + 팀장이 대면으로 해결한다.
...
2. **API 계약 문서 작성 ↔ 디자인 시안 (병렬)** — 두 트랙은 서로 막지 않는다.
   ...
   - `design/`: 화면 시안. **이 항목(프론트엔드 UI/UX 시안 작업)을 시작하기 전에
     ...
     **reference-main.html은 메인/홈 화면만 다룬다** — 목록·상세류 화면을 그릴 때는 대신
     `reference/project-management/*.html`(개별 파일 7장)을 구현 예시로 참고한다(2026-09-03
     추가, 근거: `ux-philosophy/` 기존 구현 예시가 메인 화면 하나뿐이라 목록·상세 화면을 그릴
     때 참고할 게 없었다). **필요한 화면 파일 하나만 읽는다** — 예를 들어 목록류를 만든다면
     `browse.html`만 읽으면 된다. 같은 폴더의 `project-management-bundle.html`(화면 10장을
     base64 이미지까지 인라인한 400KB 단일 파일)은 사람이 브라우저에서 인터랙션을 확인할
     때만 열고, AI가 구조 참고용으로 통째로 읽지 않는다 — 이미지 base64 인라인이 파일 읽기와
     git diff를 깨뜨리는 문제는 이 문서에도 이미 한 번 나왔다(위 "이미지 등 자산 파일" 항목,
     2026-09-02 `reference-main.html` 147KB 사례). 이 파일들은 리포 루트 `reference/`에 있는
     **고정 스냅샷**이다 — 원본은 `features/project-management/design/reference-proposal/`
     에서 담당자(유동우)가 계속 갱신하지만, 여러 기능이 동시에 구현하는 스프린트 기간 동안
     참고 기준이 흔들리지 않도록 2026-09-03 시점 버전으로 얼려 두었다(`reference/README.md`
     참고). 원본 화면 10장 중 확정 참고 대상은 7장뿐이다 — `experts.html`·`expert.html`·
     `guide.html` 3장은 ERD·PRD 근거가 없어 향후 검토 대상으로 남겨두었으니(개별 파일
     폴더에도 애초에 복사하지 않았다) 구현 근거로 쓰지 않는다. 구조·원칙 참고용으로 쓰고
     그대로 베끼지 않는다. 자세한 역할 구분은 `ux-philosophy/AGENTS.md` 참고.
   ...
   - **이미지 등 자산 파일이 필요하면 `features/{기능}/assets/{카테고리}/`에 카테고리별로 나눠
     저장하고, `design/`·`prototype/web/`에서는 상대경로로 참조한다** (예: `assets/images/`,
     `<img src="../assets/images/hero.jpg">`). 카테고리는 필요한 만큼만 만든다 — 처음엔
     `images` 하나만 있어도 된다. **`design/*.html` 안에 이미지를 base64로 욱여넣지 않는다**
     (2026-09-02 추가, 근거: `ux-philosophy/reference-main.html`을 base64 5개로 만들었더니
     337줄짜리 파일이 147KB가 되고 한 줄이 6만 자를 넘어서, 이 세션에서 파일을 읽을 때 자동으로
     잘려 표시되는 문제가 실제로 있었다 — AI가 파일을 읽기 훨씬 어려워지고, git diff도 그 긴
     base64 한 줄이 통째로 바뀐 것처럼 보여 리뷰가 안 된다). 별도 파일 + 상대경로 참조가 더
     가볍고, git에서도 이미지 자체가 바이너리 blob으로 남아 diff가 깨끗하다.
