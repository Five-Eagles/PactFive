 ...
 *
 * **카드 전체가 상세로 가는 자리다** (2026-09-04). 제목 글자만 눌리면 예산이나 기술을
 * 보다가 누르려 할 때 빗나간다. 마크업은 그대로 두고 CSS 로 제목 링크를 카드 크기까지
 * 늘렸다(`tokens.css` 의 `.pcard h3 a::after`) — 카드를 `<a>` 로 감싸면 안에 있는
 * 북마크 버튼이 링크 안의 버튼이 되어 브라우저마다 다르게 동작한다.
 *
 * 그래서 **북마크 슬롯에 들어오는 것은 링크가 아니라 버튼이어야 한다.** 늘어난 링크 위로
 * 올려 두긴 하지만, 링크 안에 링크가 겹치면 그건 CSS 로 풀 수 없다.
 */
