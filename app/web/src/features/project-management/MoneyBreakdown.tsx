 ...
 * 금액이 어디서 왔는지 보여준다
 *
 * 원본: features/project-management/prototype/web/MoneyBreakdown.tsx (d69e8e8, CR-0006 결함 2)
 *
 ...
 * ## 왜 필요한가 (CR-0006 결함 2)
 *
 * spec.md 규칙 8 은 `pricingAnalysisId` 가 있으면 **사용자가 입력한 금액을 버리고
...
import { Money } from '../../shared/ui/primitives';

/** `app/server/.../project.types.ts` 의 `BudgetSource` 와 같은 값이다 */
...
export function MoneyBreakdown({ amount, source, sourceAt, label = '예산' }: MoneyBreakdownProps) {
  return (
    <div className="card" style={{ marginBottom: 20 }}>
      <p className="caption" style={{ margin: '0 0 4px' }}>
        {label}
      </p>
      <p className="pcard__budget" style={{ margin: '0 0 6px' }}>
        <Money amount={amount} />
      </p>
      ...
      {source && (
        <p className="caption" style={{ margin: 0 }}>
          {SOURCE_TEXT[source]}
          {sourceAt && <span> · {formatDate(sourceAt)}</span>}
        </p>
      )}
