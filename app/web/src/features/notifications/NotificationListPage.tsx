...
/**
 * 원본: features/notifications/prototype/web/NotificationListPage.tsx (오민혁, PR #75/#90).
 * 로컬 `notifications.types`/`useNotifications`(파일명만 camelCase로 바꿈)를 쓰고,
 * `<style>{notificationStyles}</style>` 인라인 대신 `notifications.css` import로 바꿨다
 * (HomePage.tsx가 `home.css`를 import하는 것과 같은 이 프로젝트 관례). 로그인 링크·프로젝트
 * 보기 링크는 `<a>` → `Link`로 바꿨다 — 이 페이지는 로그인 후에만 의미가 있어 전체 새로고침이
 * 일어나면 메모리 전용 Access Token이 사라진다(NotificationBell.tsx와 같은 이유, 2026-09-09).
 */
...
const category: Record<NotificationType, string> = {
  APPLICATION_SUBMITTED: '지원',
  APPLICATION_ACCEPTED: '지원',
  APPLICATION_REJECTED: '지원',
  APPLICATION_AUTO_REJECTED: '지원',
  PROJECT_RECRUITMENT_CLOSED: '모집',
  PROJECT_CANCELED: '프로젝트',
  AGREEMENT_ACCEPTED: '금액 합의',
  AGREEMENT_REJECTED: '금액 합의',
  CONTRACT_SIGNED: '계약',
  PAYMENT_COMPLETED: '결제',
  DELIVERY_REQUESTED: '납품',
...
export function NotificationListView({ snapshot, onRefresh, onMarkRead, onMarkAllRead }: NotificationListViewProps) {
  ...
  useEffect(() => {
    ...
    // 사용자가 요청 중에 일부러 다른 곳으로 초점을 옮기지 않았을 때만 복원한다.
    if (document.activeElement === pending.button || document.activeElement === document.body) {
      const target = pending.refresh
        ? refreshRef.current
        : filter === 'all' && pending.id
          ? rowLinks.current.get(pending.id)
          : filterRef.current;
      target?.focus();
    }
  ...
  return (
    <div className="ntf-root" data-testid="notification-page">
      <main className="ntf-page">
        <p className="ntf-crumb">내 활동 / 알림</p>
        <div className="ntf-heading">
          <div>
            <h1 id="notification-heading" tabIndex={-1}>
              알림
            </h1>
            <p className="ntf-intro">프로젝트의 새로운 소식을 확인하세요.</p>
          </div>
          <NotificationBell unreadCount={sessionExpired ? null : snapshot.unreadCount} />
        </div>
        <div className="ntf-layout">
          <aside className="ntf-sidebar" aria-label="알림 요약">
            <h2>전체 안 읽은 알림</h2>
            <p className="ntf-stat" data-testid="notification-unread-count">
              {sessionExpired ? '—' : snapshot.unreadCount ?? '—'}
              <span>개</span>
            </p>
            <p>읽음 처리한 알림도 목록에 남아 있어요.</p>
          </aside>
          <section aria-label="알림 목록">
            <div className="ntf-toolbar">
              <div className="ntf-filters" role="group" aria-label="알림 필터">
                <button
                  className="ntf-button"
                  type="button"
              ...
              <div className="ntf-actions">
                ...
                >
                  {snapshot.isRefreshing ? '새로고침 중…' : '새로고침'}
                </button>
                <button
                  className="ntf-button ntf-primary"
                  type="button"
                  disabled={busy || sessionExpired || !snapshot.unreadCount}
                  onClick={(event) => {
                    setInteracted(true);
                    pendingFocus.current = { button: event.currentTarget };
                    onMarkAllRead?.();
                  }}
                  data-testid="notification-read-all"
                >
                  {snapshot.isMarkingAllRead ? '읽음 처리 중…' : '모두 읽음'}
                </button>
              </div>
            </div>
            <p className="ntf-helper">최근 100건을 보여드려요. 안 읽음 수는 전체 알림 기준입니다.</p>
            <p className="ntf-message" role="status" aria-live="polite" aria-atomic="true">
              {snapshot.message}
            </p>
            {snapshot.errorMessage && !sessionExpired && (
              <div className="ntf-error" role="alert">
                <span>{snapshot.errorMessage}</span>
                <button type="button" className="ntf-button" disabled={busy} onClick={refresh}>
                  다시 시도
                </button>
              </div>
            )}
            <div aria-busy={busy || snapshot.status === 'loading'}>
              {sessionExpired ? (
                <div className="ntf-status" data-testid="notification-session">
                  <div className="ntf-symbol">
                    <NotificationBellIcon />
                  </div>
                  <h2>다시 로그인해 주세요</h2>
                  <p>로그인 상태를 확인할 수 없어 알림을 숨겼습니다.</p>
                  <Link className="ntf-button ntf-primary" to="/login?returnTo=%2Fnotifications">
                    로그인
                  </Link>
                </div>
                ...
                <div className="ntf-status" data-testid="notification-error">
                  ...
                  <p>연결을 확인한 뒤 위의 다시 시도를 눌러 주세요.</p>
                </div>
              ) : !visible.length ? (
                <div className="ntf-status" data-testid="notification-empty">
                  <div className="ntf-symbol">
                    <NotificationBellIcon />
                  </div>
                  <h2>{filter === 'unread' ? '최근 100건에 안 읽은 알림이 없어요' : '아직 도착한 알림이 없어요'}</h2>
                  <p>
                    {filter === 'unread'
                      ? '전체 목록에서 지난 알림을 다시 확인할 수 있어요.'
                      : '프로젝트의 새로운 소식이 생기면 이곳에 알려드릴게요.'}
                  </p>
                  {filter === 'unread' && (
                    <button className="ntf-button" type="button" onClick={() => setFilter('all')}>
                      전체 알림 보기
                    </button>
                  )}
                </div>
              ) : (
                <ul className={`ntf-feed${interacted ? '' : ' ntf-enter'}`} data-testid="notification-list">
                  {visible.map((entry, index) => {
                    const read = isRead(entry.id, entry.readAt);
                    return (
                      <li
                        key={entry.id}
                        className={`ntf-row${read ? '' : ' ntf-unread'}`}
                        style={{ '--ntf-order': index } as CSSProperties}
                        data-notification-id={entry.id}
                      >
                        <span className="ntf-symbol" aria-hidden="true">
                          <NotificationBellIcon />
                        </span>
                        <div>
                          <div className="ntf-row-top">
                            <span>{category[entry.type]}</span>
                            <span className={read ? 'ntf-read-state' : 'ntf-unread-label'}>{read ? '읽음' : '안 읽음'}</span>
                          </div>
                          <h2>{entry.title}</h2>
                          <p>{entry.body}</p>
                          <div className="ntf-row-footer">
                            <time dateTime={entry.createdAt}>{formatNotificationTime(entry.createdAt)} (한국 시간)</time>
                            <div className="ntf-row-actions">
                              <button
                                type="button"
                                className="ntf-button"
                                disabled={read || busy}
                                aria-label={`${entry.title}: ${read ? '읽음' : '읽음 처리'}`}
                                onClick={(event) => {
                                  setInteracted(true);
                                  pendingFocus.current = { button: event.currentTarget, id: entry.id };
                                  onMarkRead?.(entry.id);
                                }}
                              ...
                              {isNotificationProjectLink(entry.linkUrl) && (
                                <Link
                                  ref={(node) => {
                                    ...
                                    else rowLinks.current.delete(entry.id);
                                  }}
                                  className="ntf-link"
                                  to={entry.linkUrl}
