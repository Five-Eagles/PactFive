...
| 구현 예시(참고용) | `ux-philosophy/reference-main.html`(메인/홈) · `features/project-management/design/reference-proposal/browse.html`(목록류) · `.../detail.html`(상세류, 2026-09-03 추가) | 전부 색상·컴포넌트 값의 정본은 아님. browse·detail 2종은 처음부터 design-tokens.md 토큰만 써서 reference-main.html 같은 색상 불일치는 없지만, 담당자(유동우)가 계속 갱신 중이라 상태는 "제안, 확정 아님"(`reference-proposal/README.md` 참고). (2026-09-02: 이전에 있던 레이아웃 리듬·컴포넌트 조합 참고 자료는 팀 회의에서 반려되어 제거했다 — 경위는 `docs/decisions/0012-*.md` 참고) |
| 규칙·상태 판정 | `spec.md` | |
