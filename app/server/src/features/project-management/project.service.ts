/**
 * 공개 API 구현 — 브라우저가 부르는 9종 (A-01~A-08 · A-13)
 *
 ...
 * 두 파일을 나눈 이유는 주소 체계가 다르기 때문이다 — 공개는 `/api/v1`,
 * 계약은 `/internal/v1` 이고 사용자 로그인 토큰으로는 후자에 닿을 수 없다 (규칙 49).
 *
 * ## 이 파일이 지키는 세 가지
 *
 * 1. **거래 상태는 등록 의뢰인에게만** — 그 외에는 키 자체를 넣지 않는다 (규칙 9).
 * 2. **잠금은 서버가 계산한다** — `editableFields` · `availableActions` (규칙 13).
 * 3. **모르면 잠근다** — 대기 지원 수를 읽지 못하면 0 으로 보지 않고 잠금을 유지한다 (규칙 15).
 *
 * 원본과 달라진 곳: 참조 데이터(카테고리·기술·의뢰인 프로필)를 Mock 모듈에서 직접
 * import 하지 않고 `ports.catalog` 를 거친다 (project.port.ts 주석 참고).
 */
...
import type { AuthContext, ExternalPorts, TransactionContext } from './project.port';
...
export type ProjectServiceDeps = {
  ...
  ports: ExternalPorts;
  now: () => string;
  /** 새 프로젝트 id. Prisma 도입 후에는 DB 가 만든다 */
  newProjectId: () => string;
};
...
export function createProjectService(deps: ProjectServiceDeps) {
  ...
  function mustOwn(project: ProjectRecord, auth: AuthContext): void {
    if (project.clientId !== auth.userId) {
  ...
  function validateSkills(skillIds: string[]): void {
    ...
    for (const id of skillIds) {
      if (!ports.catalog.isKnownSkill(id)) {
        fail(422, 'INVALID_SKILL', '존재하지 않는 기술입니다.', { skillId: id });
      }
      // 규칙 5 — 프리랜서가 직접 만든 기술은 프로젝트 요구 기술이 될 수 없다.
      // 검색·추천이 그 이름을 아무도 모르는 값으로 갈라진다.
      if (!ports.catalog.isOfficialSkill(id)) {
  ...
  function validateCommonFields(input: Partial<CreateProjectInput>): void {
    ...
    if (input.category !== undefined && !ports.catalog.isValidCategory(input.category)) {
  ...
  function toPublicItem(p: ProjectRecord, at: string): PublicProjectItem {
    return {
      ...
      title: p.title,
      category: ports.catalog.toCategoryRef(p.category),
  ...
  function listProjects(query: ProjectListQuery): Responded<ProjectListResponse> {
    ...
    if (query.category !== undefined && !ports.catalog.isValidCategory(query.category)) {
      fail(422, 'INVALID_CATEGORY', '존재하지 않는 카테고리입니다.', { category: query.category });
    }
    for (const skillId of query.skills ?? []) {
      if (!ports.catalog.isKnownSkill(skillId)) {
        fail(422, 'INVALID_SKILL', '존재하지 않는 기술입니다.', { skillId });
      }
    }
    ...
    if (query.skills?.length) {
      const wanted = query.skills;
      rows = rows.filter((p) => wanted.every((s) => p.skillIds.includes(s)));
    }
    if (query.minBudget !== undefined) {
      const min = query.minBudget;
      rows = rows.filter((p) => p.budgetAmount >= min);
    }
    if (query.maxBudget !== undefined) {
      const max = query.maxBudget;
      rows = rows.filter((p) => p.budgetAmount <= max);
    }
  ...
  function updateProject(
    ...
    if (
      effectiveRecruitmentStatus(project, at) === 'CLOSED' ||
      project.transactionStatus !== 'NONE'
    ) {
    ...
    const editable = editableFieldsOf(project, at);
    const locked = Object.keys(input).filter((field) => !editable.includes(field));
    if (locked.length > 0) {
      fail(409, 'PROJECT_EDIT_LOCKED', '지원자가 있어 예산과 일정은 변경할 수 없습니다.', {
        lockedFields: locked,
        pendingApplicationCount: project.pendingApplicationCount,
      });
    }
  ...
  function deleteProject(auth: AuthContext | null, projectId: string): Responded<null> {
    ...
    if (project.pendingApplicationCount > 0) {
      fail(409, 'PROJECT_DELETE_HAS_APPLICATIONS', '지원자가 있어 삭제할 수 없습니다.', {
        pendingApplicationCount: project.pendingApplicationCount,
      });
    }
    if (project.transactionStatus !== 'NONE' && project.transactionStatus !== 'CANCELED') {
  ...
  async function closeRecruitment(
    ...
    if (project.recruitmentStatus === 'CLOSED') {
      return {
        ...
        body: {
          ...
          closedAt: project.recruitmentClosedAt ?? at,
        },
      };
    }
    ...
    if (project.deadlineNotifiedAt === null) {
    ...
    if (project.transactionStatus === 'CANCELED') {
      return {
        ...
        body: {
          ...
          canceledAt: project.canceledAt ?? at,
          postActions: { applicationRejection: 'NOT_NEEDED', contractInvalidation: 'NOT_NEEDED' },
        },
      };
    }
    // 규칙 27 — 결제가 시작됐으면 막는다. 돈이 움직인 뒤의 취소는 환불 문제가 된다.
    if (project.paymentPendingAt !== null) {
      fail(409, 'PROJECT_CANCEL_AFTER_PAYMENT', '결제가 시작되어 취소할 수 없습니다.', {
        paymentPendingAt: project.paymentPendingAt,
      });
    }
    // 규칙 28
    if (project.transactionStatus !== 'NONE' && project.transactionStatus !== 'CONTRACT_PENDING') {
  ...
  function reopenRecruitment(
    ...
    if (project.recruitmentStatus === 'OPEN') {
      return {
        ...
        body: {
          ...
          transactionStatus: project.transactionStatus,
          recruitmentStartAt: project.recruitmentStartAt ?? project.createdAt,
          recruitmentDeadlineAt: project.recruitmentDeadlineAt,
    ...
    if (project.transactionStatus !== 'NONE' || project.canceledAt !== null) {
      fail(
        409,
        'PROJECT_TRANSITION_CONFLICT',
        '거래가 있거나 취소된 프로젝트는 재모집할 수 없습니다.',
        { transactionStatus: project.transactionStatus },
      );
    }
    ...
    if (
      ...
      fail(409, 'PROJECT_VERSION_CONFLICT', '다른 곳에서 먼저 변경되었습니다.', {
        ...
        current: project.projectVersion,
      });
    }
