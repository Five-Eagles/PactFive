 ...
 *
 * 원본: features/engagement/prototype/server/bookmark.service.ts (3e4977e)
 *
 * ## 이 파일이 지키는 세 가지
 *
...
import type { EngagementPorts, ProjectCardData } from './bookmark.port';
...
export function createEngagementService(deps: EngagementServiceDeps) {
  ...
  async function addBookmark(
    ...
    try {
      ...
      if (!(error instanceof BookmarkAlreadyExistsError)) throw error;
  ...
  async function listBookmarks(
    ...
    const items: BookmarkItem[] = alive.slice(start, start + pageSize).map((bookmark) => {
      const project = cards.get(bookmark.projectId) as ProjectCardData;
      return {
  ...
  /* ═══════════ 3-2. 저장한 프로젝트 id (규칙 35·36, CR-0008) ═══════════ */
  ...
  async function getRecommendations(projectId: string): Promise<Responded<RecommendationResponse>> {
    ...
    const ranked = [...candidates].sort((a, b) => {
      const tierA = tierOf(a, base.category.category, skillIds);
      const tierB = tierOf(b, base.category.category, skillIds);
        ...
        items: ranked.slice(0, RECOMMENDATION_COUNT).map((c): RecommendedItem => {
          ...
          return {
            ...
            matchedSkills: c.skills
              .filter((s) => skillIds.includes(s.skillId))
              .map((s) => s.displayName),
          };
        }),
      },
