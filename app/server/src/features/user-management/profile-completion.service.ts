...
/** 원본: features/user-management/prototype/server/profile-completion.service.ts (오민혁, PR #89). */

// ERD E-27/D-91 및 현 Prisma enum(BusinessField/ProjectCategory). 옛 ETC 설명을 근거로
// 폐기된 값을 다시 허용하지 않는다.
...
function hasValidProfileShape(snapshot: ProfileCompletionSnapshot): boolean {
  ...
  if (
    !snapshot.profile ||
    snapshot.profile.userId !== snapshot.userId ||
    !isNullableString(snapshot.profile.completedAt)
  )
    return false;
  if (snapshot.role === 'CLIENT') {
    return (
      isNullableString(snapshot.profile.companyName) &&
      isNullableString(snapshot.profile.businessField) &&
      isNullableString(snapshot.profile.businessFieldEtc)
    );
  }
  return (
    isNullableString(snapshot.profile.primaryCategory) &&
    (snapshot.profile.careerYears === null || typeof snapshot.profile.careerYears === 'number') &&
    Array.isArray(snapshot.profile.skills) &&
    Array.from(snapshot.profile.skills).every(
      (skill) => skill && typeof skill.skillId === 'string' && typeof skill.isActive === 'boolean',
    )
  );
}
...
function getMissingFields(snapshot: ProfileCompletionSnapshot): ProfileMissingField[] {
  ...
  if (snapshot.role === 'CLIENT') {
    ...
    if (
      ...
      missingFields.push('CLIENT_COMPANY_NAME');
    ...
    if (
      typeof profile?.careerYears !== 'number' ||
      !Number.isInteger(profile.careerYears) ||
      profile.careerYears < 0 ||
      profile.careerYears > 32_767
    )
      missingFields.push('FREELANCER_CAREER_YEARS');
    ...
    async getProfileCompletion(userId): Promise<ProfileCompletion> {
      ...
      try {
        ...
        if (
          ...
          (snapshot.role !== 'CLIENT' && snapshot.role !== 'FREELANCER')
        )
          return unavailable();
