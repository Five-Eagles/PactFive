...
## 14. Skeleton 컴포넌트 스펙 (2026-09-14 추가)

§8 Feedback 목록에 이름만 있던 `Skeleton`을 실제로 정의한다. 배경 — 로딩 중 화면에서 텍스트
문구(`"불러오는 중입니다…"`)가 마운트·언마운트되며 화면이 점멸하는 문제가 확인됐다(의사결정
근거와 화면별 적용 매핑은 `docs/decisions/0018-loading-skeleton-adoption.md` 참고). 이 절은 그
문서가 가리키는 정본 시각 규격이다. **새 색·새 반경·새 그림자 토큰은 만들지 않는다** — 아래는
모두 §3에 이미 있는 토큰의 조합이다.

### 모양 3종

| 이름 | 쓰는 곳 | 크기·반경 |
|---|---|---|
| 글줄형(`line`) | 상세·패널의 텍스트 자리 | 높이는 대체할 타이포그래피의 `lineHeight`와 같게, `primitive.radius.sm`(4px) |
| 카드형(`card`) | 목록·그리드의 카드 자리 | `componentTokens.card`와 동일한 비율·반경(`primitive.radius.lg`, 12px) |
| 목록행형(`row`) | 테이블·리스트의 한 행 자리 | 높이 `componentTokens.table.rowHeight`(56px), 반경 `primitive.radius.md`(8px) |

세 모양 모두 내부에 같은 시머 처리를 쓰되, 실제 콘텐츠가 차지할 공간을 미리 예약해 레이아웃이
밀리지 않게 하는 것이 목적이다 — 색이나 애니메이션보다 "크기가 맞는가"가 더 중요하다.

### 색

- 바탕: `semantic.color.surface.muted`(#EEF1F5)
- 시머 하이라이트: `semantic.color.surface.default`(#FFFFFF)를 40% 불투명도로 겹친 그라데이션 —
  새 색을 추가하지 않고 기존 흰색에 투명도만 적용한다.
- 다른 색(teal·rust 등 캠페인/상태 색)은 스켈레톤에 쓰지 않는다 — 로딩 상태는 항상 중립색이다
  (§12 금지 패턴 "상태를 색 하나로만 구분" 원칙과 같은 이유로, 로딩은 애초에 상태 정보를 전달할
  필요가 없다).

### 모션

- 시머는 왼쪽에서 오른쪽으로 1.4초 동안 쓸고 지나가는 그라데이션 sweep이며, 끊김 없이
  반복(loop)한다. 이 1.4초는 §13의 전환용 `primitive.motion.duration`(0~240ms) 값과 성격이
  다르다 — 전환은 "상태가 바뀌는 순간"에 한 번 쓰는 값이고, 시머는 "로딩이 끝날 때까지 계속
  도는" 배경 효과이기 때문에 별도 값을 쓴다. 이 컴포넌트 전용 값으로만 존재하며 다른 곳에
  재사용하지 않는다.
- easing은 `primitive.motion.easing.standard`를 재사용한다.
- **최초 마운트 stagger(§13)를 스켈레톤 자체에는 적용하지 않는다** — 스켈레톤은 로딩이 시작되는
  즉시 전부 동시에 나타나야 한다. "스켈레톤이 하나씩 나타나는 로딩"은 오히려 로딩이 두 번
  일어나는 것처럼 보인다.
- `prefers-reduced-motion: reduce`에서는 시머를 끄고 `semantic.color.surface.muted` 단색 정지
  상자만 보여준다 — §13 접근성 절과 동일한 예외 처리다.

### 접근성

- 스켈레톤을 감싸는 컨테이너에 `role="status"`와 `aria-busy="true"`를 준다.
- 화면에 보이는 텍스트 문구는 없애되, 보조기술 전용으로 `sr-only`(시각적으로 숨김) 텍스트
  `"불러오는 중입니다"`를 컨테이너 안에 남긴다 — §10 "비동기 상태 변화는 보조 기술에 전달한다"
  요구사항을 계속 만족시키기 위해서다. 즉 사람 눈에는 상자만 보이고, 스크린리더 사용자에게는
  기존과 같은 안내 문장이 그대로 전달된다.
- 스켈레톤 상자 자체는 장식(decorative)이므로 `aria-hidden="true"`를 준다 — 상자 하나하나를
  스크린리더가 읽을 필요는 없다.

### 금지

- 스켈레톤 위에 실제 텍스트나 아이콘을 겹쳐 쓰지 않는다(§12 금지 패턴과 같은 이유 — 정보
  전달과 장식을 섞지 않는다).
- 스켈레톤 모양이 실제 콘텐츠 크기와 20% 이상 차이 나면 안 된다 — 크기가 다르면 콘텐츠가
  도착했을 때 레이아웃이 밀리는 문제(스켈레톤을 쓰는 애초의 목적)가 그대로 재현된다.
...
- [ ] 로딩 중 화면이 텍스트 마운트/언마운트로 점멸하지 않고 스켈레톤 상자로 대체되어 있다(§14).
- [ ] 스켈레톤 컨테이너에 `role="status"`·`aria-busy="true"`가 있고, 스크린리더 전용 안내
      문구가 남아 있다(§14).
- [ ] `prefers-reduced-motion`에서 스켈레톤 시머가 꺼지고 정지 상자만 보인다(§14).
