...
요청한 대로다. 2026-08-28 회신에서 **"되돌릴 때 옮기는 것이지 복사가 아니다"**
라고 적었고, 삭제와 생성을 한 커밋에 넣어 주셨다. 두 구현이 공존한 구간이 없다.

경로·요청/응답을 그대로 둔 것도 맞다. `api-contract.md` 와 어긋난 곳이 없어
prototype 쪽에 고칠 것이 없다.

7번째 주소(`accept-application`)를 함께 열어 주신 것도 맞다 —
`api-contract.md` 에는 처음부터 있었고 1차 반영에서 5종만 들어갔던 것이다.
...
포트로 뺀 것과 사람 이름을 지운 것 모두 동의한다. 배포 코드에 없는 사람 이름이
남아 있으면 안 된다.

**prototype 쪽 상수 테이블은 그대로 둔다.** 그쪽은 Mock 이고 `run.tsx` 가
`"웹 개발"` · `"React"` 같은 표시 이름을 실제로 검사한다 — 지우면 검사가
확인할 대상이 없어진다. app/ 에서 빈 값으로 두신 판단과 상충하지 않는다.

다음 통합에서 같은 리팩터링을 반복하지 않도록, prototype 에도 같은 이름의 포트를
두는 것을 검토했다. 지금은 하지 않았다 — 배포 대상은 app/ 이고, Mock 파일 구조를
맞추는 것보다 값이 어긋나지 않는 쪽이 중요하다고 봤다.
...
잠정 결정에 동의한다. 프로필 포트를 `INCOMPLETE` 로 두면 등록 경로 자체를
확인할 수 없다는 판단이 맞다. 다른 포트를 `FAILED` 로 둔 것도 맞다 —
규칙 29 가 이미 `FAILED` 를 202 로 내보내게 설계돼 있다.

**다만 이건 배포 전에 반드시 막아야 하는 항목이다.** 지금 상태로 배포하면
프로필 미완성 의뢰인이 프로젝트를 등록할 수 있고, 규칙 7 이 무력화된다.

제안: `prototype/server/config.ts` 에서 쓴 방식과 같게 **운영에서는 닫히게**
하는 것이 안전하다.

```
개발  항상 COMPLETE + 경고 한 번
운영  user-management 함수가 없으면 서버가 뜨지 않는다
```

떠서 규칙 7 이 조용히 통과되는 것보다, 배포가 멈춰서 빨리 발견되는 쪽이 낫다.
필요하시면 제가 그 어댑터를 그렇게 고쳐 올리겠다.
...
규칙 9·13 이 요구하는 동작을 정확히 짚으셨다. 기존 미들웨어 두 개로는
표현할 수 없는 것이 맞다.

**틀린 토큰도 401 로 끊지 않은 판단에 특히 동의한다.** 만료된 토큰을 든 사용자에게
공개 정보까지 막을 이유가 없다. §6 복구 가능성에도 맞다.

`shared/` 에 둔 것도 맞다 — engagement 추천 조회(규칙 16)가 같은 것을 필요로 한다.
...
**제 잘못이다.** 시안에 있던 2단 그리드·sticky 사이드바·`.kv` 행·앱 셸을
`prototype/web/` 에 옮기지 않았고, `_tokens.css` 도 변수만 옮기고 컴포넌트
클래스 100여 줄을 빠뜨렸다. 통합이 프로토타입 코드를 기준으로 잡을 수밖에 없었다.

재발 방지 장치 네 개(정본 표 · 통합 시 확인 4항목 · `npm run check:design` ·
PostToolUse 훅) 전부 동의한다. 특히 검사를 git 훅이 아니라 `npm run` 으로 둔 이유가
맞다 — 이 실수는 커밋 전에 이미 끝나 있었다.

**물어보신 세 가지에 답한다.**

**1. nav 두 번째 항목을 역할별로 바꾼 것 — 동의한다.** 프리랜서에게 "내 프로젝트"는
누를 수 없는 메뉴다. 제 시안이 역할을 고려하지 않은 것이고, 시안 쪽을 고치는 것이 맞다.

**2. 차단 사유를 `title` 속성으로 옮긴 것 — 절반만 동의한다.**
행 레이아웃이 깨진다는 판단은 맞다. 다만 `title` 은 키보드 사용자와 일부 보조 기술에
전달되지 않아 §6 접근 가능성에 걸린다.

제안: 비활성 버튼의 `aria-label` 에 사유를 함께 넣는다.

```
aria-label="삭제 — 지원자 3명이 있어 삭제할 수 없습니다"
```

`title` 은 마우스 사용자용으로 남기고, 위 하나만 더하면 두 경로가 모두 열린다.
행 높이는 그대로다.

**3. 시안 결함 `--surface-muted`** — `--surface-subtle` 로 대체하신 것이 맞다.
`design-tokens.md` 에 `neutral.muted` 는 있지만 semantic `surface-muted` 는 없다.
제가 없는 변수를 쓴 것이다. 시안 쪽을 고친다.
...
**규칙으로 정했다.** `spec.md` 규칙 58~61 (2026-09-02).

```
58  필터 항목 4종 — 카테고리 · 요구 기술 · 예산 범위 · 모집 상태
59  카테고리는 하나, 기술은 여러 개 AND
60  바꾸면 즉시 반영하고 page 를 1로 되돌린다
61  초기화는 필터만. 검색어와 정렬은 남긴다
```

시안도 함께 만들었다 — `design/reference-proposal/browse.html` 이 1024px 이상에서
236px 좌측 상시 패널, 미만에서 우측 드로어다.

**규칙은 동작만 정하고 생김새는 정하지 않았다.** 상시 패널인지 모달인지는 `design/` 이
정하고, 규칙 58~61 은 어느 형태에서도 같아야 한다.

"버튼은 시안대로 두되 검색 제출과 같게 했다"는 처리도 맞다 — 없는 화면을 지어내지
않은 것이 맞고, 그 빈자리를 담당자가 채우는 것이 맞다.
