...
| 확인 필요 | 조준영 (contracts-payments) — **문서 상태만 정리, 조준영 회신은 여전히 없음** |
| 상태 | **반영 완료 (코드), 회신 대기 (2026-09-09, 팀장 문서 정리).** |
| 관련 | spec.md 규칙 39·47 · api-contract.md `restore-pre-contract` |

> **문서 상태 정리 (2026-09-09, 팀장).** `project-contract.service.ts`(:405~425)에
> 제안대로 그대로 있다 — `restoredFields`에 `acceptedApplicationId` 항상 포함,
> `paymentPendingAt`은 값이 있었을 때만 포함. **조준영의 실제 회신은 아직 못 받았다** —
> "제안" 상태를 "반영 완료"로 바꾸는 게 아니라, 코드는 이미 이 CR대로 동작 중이라는
> 사실만 문서에 남긴다. 규칙 39·47과 충돌 없음(자동 거절된 지원자는 되살리지 않음,
> 프로젝트 쪽 수락 표시만 지움).
