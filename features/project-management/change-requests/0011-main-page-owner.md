...
> **닫음 (2026-09-09, 팀장).** `prd-v6.4.md:3281` 표 행을 분리해 `홈/랜딩` 담당을
> 유동우로, `로그인 · 프로필`은 오민혁으로 유지했다. `:3439` mermaid 플로우차트의
> `HOME` 노드도 `(유동우, CR-0011)`로 갱신. `docs/domain/api-spec/`은 이 저장소에
> 없어 요청 2번은 대상 없음.
