...
function requireEmptyPublicInput(request: Request): void {
  ...
  if ((queryIndex !== -1 && request.originalUrl.length > queryIndex + 1)
    || Object.keys(request.query).length !== 0) throw invalidNotificationInput();
...
export function createNotificationController(service: NotificationService) {
  ...
  return {
    ...
    markNotificationRead: handle((auth, request) => service.markNotificationRead(auth, String(request.params.notificationId))),
