...
원본 `useAuth`의 소비자별 상태 분리를 공유 메모리 구독으로 보정했다. 로그인·계정 전환·
로그아웃이 헤더/폼에 같이 반영되고, 늦은 복원/로그아웃 응답은 최신 계정을 덮지 않는다.
기본 자동 검증 106 PASS, 별도 React 마운트 훅 검증 12 PASS; app 통합/배포 검증은 아니다.
프로필은 이식할 독립 시안·React 화면·저장 API가 **원본에도 없음**을 확인했다. 기존 내부 완성도
포트와 인증 사용자 요약 API를 분리하고, 파일 매핑·최소 요약/상세 편집 범위 결정·통합 회귀를
`change-requests/0001-profile-completion-integration.md`의 후속 절에 정리했다. 새 UI/API나
게이트 활성화는 하지 않았다. 디자인 프롬프트는 참고했으나 재설계는 요청 범위가 아니므로
기존 시안/표현을 변경하지 않았다.
