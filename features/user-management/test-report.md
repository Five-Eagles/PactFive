...
- [x] user-management `prototype/` 범위 TypeScript 검사 통과 (scoped tsc: PASS)

35건은 `spec.md` 규칙별 Mock 테스트 23건, high-fi 필수 텍스트 SSR 정적 렌더 검사 11건,
...
|---|---|---|
| 1 | `R01` — 입력·확인 token 검증, 확인 전 사용자/세션/토큰 미생성, 활성 세션 충돌 선검사, intent 저장 실패 보상, 10분/30일 가입 복구·변조·만료·재시도·직접 우회 차단을 Mock으로 확인 | 통과 (Mock·인메모리) |
...
| 8 | `R08` — GOOGLE/KAKAO만 허용하고 서비스가 `AuthProvider` 포트를 사용하며 Supabase SDK를 직접 import하지 않음을 확인 | 통과 (Mock·정적 검사) |
...
| 17 | `R17` — Origin 선검사, Refresh 쿠키 기준 멱등 로그아웃, Bearer 없이 로컬 세션 폐기, 저장소 오류를 명시적 503으로 분류하면서 모든 인증 쿠키 제거, Origin 거부 전 무변경, 동일 탭 mutation 직렬화, 확정 401의 epoch 무효화와 epoch 뒤 지연 Restore/최초 보호 응답/Refresh/재시도/401 결과 미게시·미이동을 확인 | 부분 통과 (Mock·인메모리·controller·웹 유틸), durable 장애 복구 제외 |
...
- `supabase-auth.adapter.ts`는 현재 `AUTH_PROVIDER_NOT_READY`로 fail-closed하는 자리표시자다. 실제
  Supabase Auth, Google/Kakao 공급자, 실제 Postgres 저장소와는 연결하지 않았다.
