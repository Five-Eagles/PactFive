...
export type MemoryLifecycleSnapshot = TransactionLifecycleSnapshotReader & {
  get(): TransactionLifecycleSnapshot;
  patch(partial: Partial<TransactionLifecycleSnapshot>): void;
};

/** 테스트용 재조회 스냅샷. 원장 행을 대신하지 않는다. */
export function createMemoryLifecycleSnapshot(
  initial: TransactionLifecycleSnapshot,
): MemoryLifecycleSnapshot {
  let current: TransactionLifecycleSnapshot = { ...initial };
  return {
    async read(projectId: string) {
      if (current.projectId !== projectId) {
        throw new Error(`lifecycle snapshot is for ${current.projectId}, not ${projectId}`);
      }
      return { ...current };
    },
    get() {
    ...
    patch(partial: Partial<TransactionLifecycleSnapshot>) {
